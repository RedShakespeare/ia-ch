...
TEST_CASE("Saving and loading the game")
{
    ...
    // ---------------------------------------------------------------------
    {
        ...
        REQUIRE(inv.item_in_slot(SlotId::wpn)->data().id == item::Id::electric_gun);

        REQUIRE(map::g_player->unarmed_wpn().id() == item::Id::player_punch);

        REQUIRE(inv.item_in_slot(SlotId::body)->data().id == item::Id::armor_asb_suit);
