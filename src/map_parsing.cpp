...
{
    ASSERT(m_parse_terrain == ParseTerrain::yes || m_parse_mobs == ParseMobs::yes || m_parse_actors == ParseActors::yes);
    ...
    for (int x_outer = 0; x_outer < dims.x; x_outer++) {
        for (int y_outer = 0; y_outer < dims.y; y_outer++) {
            ...
            for (int d = dist_interval.min;
                ...
                P p1(std::min(dims.x - 1, x_outer + d), std::min(dims.y - 1, y_outer + d));
