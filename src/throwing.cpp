...
void player_throw_lit_explosive(const P& aim_cell)
{
    ...
    item::Explosive* const explosive = actor::player_state::g_active_explosive.get();
    ...
    for (size_t path_idx = 1; path_idx < path.size(); ++path_idx) {
        ...
        if (actor_here &&
            ...
            if (att_result >= ActionResult::success) {
                ...
                if (player_see_cell) {
                    const Color hit_color = is_potion ? item_color : colors::light_red();
                ...
                if (att_data.dmg > 0) {
                    ...
                    actor_here->become_aware_player(actor::AwareSource::attacked);
                }
