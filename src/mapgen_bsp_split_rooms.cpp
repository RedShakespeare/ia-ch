...
static bsp::BlockedSplitPositions find_blocked_split_positions(const Room& room)
{
        ...
        const Fraction& chance_to_split_room)
{
        ...
        for (size_t i = 0; i < nr_original_rooms; ++i) {
                ...
                ASSERT(*(std::end(map::g_room_list) - 2) == new_rooms[0]);
                ASSERT(*(std::end(map::g_room_list) - 1) == new_rooms[1]);
