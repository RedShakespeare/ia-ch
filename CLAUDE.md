...
- **Version control** — This repo is managed with **jj** (Jujutsu, `jj 0.43.0`), colocated with the underlying git repo. Use `jj` for all version-control operations (log, diff, describe, new, abandon, rebase, etc.) instead of `git`. The git remote/branches are still available via `jj git` subcommands when needed. Commits are described with `jj describe` (not `git commit`); the working copy is an implicit revision (`@`) and does not need to be committed to save changes — describe it when its message is final. Use `jj log -r '::@'` to see history leading to the working copy.
